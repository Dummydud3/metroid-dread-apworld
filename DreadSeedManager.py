#!/usr/bin/env python3
"""
Metroid Bread Archipelago - Seed Manager GUI
...
try:
    from worlds.metroid_bread.rdvgame_export import extract_from_ap_output
except ImportError:
...
class DreadSeedManager(tk.Tk):
    """Main GUI window for Metroid Bread seed management"""
    ...
    def __init__(self):
        ...
        self.title("Metroid Bread Archipelago - Seed Manager")
    ...
    def create_basic_settings(self):
        ...
        self.game_goal_var = tk.StringVar(
            value=self.settings.get("Metroid Bread", {}).get("game_goal", "defeat_raven_beak")
        )
        ...
        self.required_dna_var = tk.IntVar(
            value=self.settings.get("Metroid Bread", {}).get("required_dna", 0)
        )
        ...
        self.dna_placement_var = tk.StringVar(
            value=self.settings.get("Metroid Bread", {}).get("dna_placement", "prefer_emmi")
        )
        ...
        self.door_lock_var = tk.StringVar(
            value=self.settings.get("Metroid Bread", {}).get("door_lock_rando", "vanilla")
        )
        ...
        self.transport_var = tk.StringVar(
            value=self.settings.get("Metroid Bread", {}).get("transport_rando", "off")
        )
        ...
        for key, label in progressive_items:
            row += 1
            var = tk.BooleanVar(value=self.settings.get("Metroid Bread", {}).get(key, True))
        ...
        row += 1
        self.early_morph_var = tk.BooleanVar(value=self.settings.get("Metroid Bread", {}).get("early_morph_ball", False))
        ...
        start_val = self.settings.get("Metroid Bread", {}).get("starting_location", "default")
        ...
        row += 1
        self.death_link_var = tk.BooleanVar(value=self.settings.get("Metroid Bread", {}).get("death_link", False))
    ...
    def create_tricks_settings(self):
        ...
        self.reverse_grapple_var = tk.BooleanVar(value=self.settings.get("Metroid Bread", {}).get("reverse_grapple_block", False))
        ...
        for category, trick_list in tricks:
            ...
            for key, label, levels, tooltip in trick_list:
                ...
                raw = self.settings.get("Metroid Bread", {}).get(key, "disabled")
    ...
    def create_item_settings(self):
        ...
        for key, label, min_val, max_val, default in items:
            ...
            var = tk.IntVar(value=self.settings.get("Metroid Bread", {}).get(key, default))
...
appear in your seed. Lower values = harder game."""
    ...
    def get_current_config(self) -> Dict[str, Any]:
        ...
        for key, var in self.progressive_vars.items():
            config["Metroid Bread"][key] = var.get()
        ...
        config["Metroid Bread"]["reverse_grapple_block"] = self.reverse_grapple_var.get()
    ...
    def load_config_to_ui(self):
        ...
        dread_config = self.settings.get("Metroid Bread", {})
        ...
        try:
            ...
            def run_full_generation():
                try:
                    ...
                    rdvgame_script = self.ap_root / "worlds" / "metroid_bread" / "rdvgame_export.py"
                    cmd = [
                        sys.executable,
                        str(rdvgame_script),
                        str(seed_folder),
    ...
    def show_about(self):
        ...
        messagebox.showinfo(
            "About",
            "Metroid Bread Archipelago - Seed Manager\n\n"
            "A GUI tool for creating YAML configurations and generating seeds\n"
            "for Metroid Bread multiworld randomizer.\n\n"
            "Features:\n"
    ...
    def show_docs(self):
        """Show documentation"""
        docs_path = self.ap_root / "worlds" / "metroid_bread" / "CLIENT_GUIDE.md"
        if docs_path.exists():
            import webbrowser
            ...
            messagebox.showinfo(
                ...
                "Located in: worlds/metroid_bread/"
            )
