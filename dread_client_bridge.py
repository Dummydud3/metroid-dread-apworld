...
""".strip()
    ...
    try:
        from worlds.metroid_bread import tracker_gate_events as _gate_mod
    except Exception:
        try:
...
function RL.CapturePlayerWorldPos()
    -- scenario;x;y for OOL death tips. Prefer GetScenarioID (GetCurrentScenarioID
    -- often nil in probes). Capture while the player actor still exists.
    local scen = ""
    pcall(function()
...
function RL.MarkLocalDeath(reason)
    ...
    -- Capture world pos NOW (post-death Remote Lua GetPlayer often fails → OOL
    -- tips never fired). Client parses "AP_DEATH: Player died|scen;x;y".
    local pos = ""
    ...
    -- Scenario / flags for CauseOfDeath empirical mapping (PlayReport is Ryujinx-side).
...
-- Empirical CauseOfDeath mapping: PlayReport fields are NOT on Blackboard/Lua.
-- This probe only returns scenario + death flags for correlating Ryujinx logs.
-- Client scrapes ServicePrepo JSON (CauseOfDeath/WhichGrab/WhichBoss/Map).
function RL.GetDeathCauseProbe()
    ...
    local deaths = RL.GetPlayerDeathCount()
    local gen = RL.DeathPollGeneration or 0
    -- Explicit nil probes so empirical runs document "not Lua-readable".
...
-- AreaBox unlock smoke lives in system/scripts/ap_map_unlock_region.lua (DoFile on
-- /map_unlock_region) — not inlined here: RL.MapUnlockRegionSmoke alone is >4096 bytes.
...
# Local deaths in an AP-unreachable area (same area names as reachable minimap).
...
# AP_DEATH log from MarkLocalDeath: "AP_DEATH: Player died" or "...died|scen;x;y"
...
do
  local ok, err = pcall(function()
    ...
    -- Pin tip 0 + count=1 BEFORE kill so ArmCarouselRefresh is live before
    -- game-over → A → checkpoint TipRefresh / SetForcedTooltip ForceRefresh.
    -- OdrTip 0.5.9: SetTipText/Count/Order store + re-arm only (no TipFill
    -- ForceRefresh; soft-fail keeps arm if TipRefresh no-ops early).
    -- Do NOT call SetForced here — that can stamp mid–game-over without a
    -- real PoolBuild; LoadGame/OnLoad ForceNextTip paints at checkpoint.
{tip_pin}
    ...
    -- Empirical CauseOfDeath: MarkLocalDeath skipped when DeathFromRemote.
